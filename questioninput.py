...
<<<<<<< HEAD
        ...
        response = model.generate_content("請根據環境資料:"+result+"回答使用者的問題，請回答給使用者完整且詳細的句子，盡量不要使用標點符號和空格，並且回答問題時請以「您」稱呼使用者。問題:"+question)
        print(response.text)
=======
        #輸入data
...
for keyword in keywordc:
    if keyword in distribution:
        ...
        q=model.generate_content("請幫我進行文字的校正，校正的部分主要是人名:[莊佩蓁、黃樺裕、羅詠琳]三個名字，等等輸入的問題可能人名中會有錯字，例如可能輸入值為:「我的前方有裝配針嗎」，就請幫我輸出:「我的前方有莊佩蓁嗎」，另外兩個人名同理，請注意，人名只需要校正同音字，這是你剛剛的錯誤案例:在這個案例中，校正前為「我的前方有誰」你的校正回答為:「我的前方有莊佩蓁嗎」，請注意，這是錯的!案例中校正前的輸入值並沒有音同莊佩蓁、黃樺裕、羅詠琳三個人名的詞，因此不需進行校正，最終輸出和原來一樣為:「我的前方有誰」。請注意不要再犯類似的錯誤。除了人名需要校正以外，輸入值的其他字請保持原樣。接下來為輸入值:"+question).text
